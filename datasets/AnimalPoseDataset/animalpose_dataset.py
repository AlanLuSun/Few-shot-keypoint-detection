...
def predefined_auxiliary_paths():
    ...
    path1 = [
        # [keypoint_types.index('l_eye'), keypoint_types.index('l_ear')],
        # [keypoint_types.index('l_eye'), keypoint_types.index('nose')],
        [keypoint_types.index('nose'), keypoint_types.index('l_ear')],
        # [keypoint_types.index('r_eye'), keypoint_types.index('r_ear')],
        # [keypoint_types.index('r_eye'), keypoint_types.index('nose')],
        [keypoint_types.index('nose'), keypoint_types.index('r_ear')],
        # [keypoint_types.index('nose'), keypoint_types.index('throat')],
        # [keypoint_types.index('throat'), keypoint_types.index('withers')],
        # [keypoint_types.index('withers'), keypoint_types.index('tail')],
        # [keypoint_types.index('l_f_leg'), keypoint_types.index('l_f_knee')],
        # [keypoint_types.index('l_f_knee'), keypoint_types.index('l_f_paw')],
        [keypoint_types.index('l_f_leg'), keypoint_types.index('l_f_paw')],
        # [keypoint_types.index('r_f_leg'), keypoint_types.index('r_f_knee')],
        # [keypoint_types.index('r_f_knee'), keypoint_types.index('r_f_paw')],
        [keypoint_types.index('r_f_leg'), keypoint_types.index('r_f_paw')],
        # [keypoint_types.index('l_b_leg'), keypoint_types.index('l_b_knee')],
        # [keypoint_types.index('l_b_knee'), keypoint_types.index('l_b_paw')],
        [keypoint_types.index('l_b_leg'), keypoint_types.index('l_b_paw')],
        # [keypoint_types.index('r_b_leg'), keypoint_types.index('r_b_knee')],
        # [keypoint_types.index('r_b_knee'), keypoint_types.index('r_b_paw')],
        [keypoint_types.index('r_b_leg'), keypoint_types.index('r_b_paw')],
    ]
...
def AnnotationPrepare(image_roots, keypoint_annotation_paths, anno_save_root, anno_save_name=None):
    ...
    for ind in range(len(total_samples)):
        ...
        for i, kp_type in enumerate(KEYPOINT_TYPES):
            x, y = total_samples[ind]['keypoints'][i][0], total_samples[ind]['keypoints'][i][1]
            flag = x < 0 or x >= w or y < 0 or y >= h
            if flag:
                total_samples[ind]['keypoints'][i][0] = 0
                total_samples[ind]['keypoints'][i][1] = 0
                total_samples[ind]['keypoints'][i][2] = 0
...
#     def __init__(self, keypoint_annotation_path, N_way, K_shot, M_queries, kp_category_set=None, order_fixed=False, vis_requirement='full_visible', least_support_kps_num=3, least_query_kps_num=3):
#         '''
...
#             self.fixed_candidates_ids = self.acquire_image_candidates_ids()
#
#         self.supports = None
#         self.queries = None
#
...
#
#     def acquire_image_candidates_ids(self):
#         candidates_ids = set()
#         for kp_type in self.support_kp_categories:
#             candidates_ids.update(set(self.imagelist[kp_type]))
...
#             num_temp = len(self.imagelist[self.support_kp_categories[0]])
#             for kp_type in self.support_kp_categories:
#                 length = len(self.imagelist[kp_type])
#                 if  length <= num_temp:
#                     num_temp = length
...
#             for i in self.imagelist[type_with_min_num]:
...
#                     kp_id = KEYPOINT_TYPE_IDS[kp_type]
#                     if each_sample['keypoints'][kp_id][2] == 0:  # invisible
...
class AnimalPoseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        keypoints = each_sample['keypoints']
        visible_bounds = each_sample['bbx']  # [h, w, xmin, ymin]
...
class AugmentDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # sample keypoints
        bbox   = np.array([visible_bounds[2], visible_bounds[3], visible_bounds[1], visible_bounds[0]], np.float)  # [xmin, ymin, w, h]
        bxmin, bymin, bw, bh = bbox
        bbox[0], bbox[1] = max(0, bxmin), max(0, bymin)  # secure our bbox is within the image
        bbox[2], bbox[3] = min(bw, w-bbox[0]), min(bh, h-bbox[1])
        bbox_origin = np.copy(bbox)  # [xmin, ymin, w, h]
...
def save_episode_before_preprocess(episode_generator, episode_num=0, delete_old_files=True, draw_main_kps=True, draw_interpolated_kps=False, interpolation_knots=None, interpolation_mode=3, path_mode='predefined', root_postfix=""):
    ...
    for sample_i, each_sample in enumerate(episode_generator.supports):
        ...
        if draw_main_kps:
            ...
            # image_out = draw_instance(image_path, keypoints_dict2, KEYPOINT_TYPES, hightlight_keypoint_types=episode_generator.support_kp_categories, save_root=None, is_show=False)
            # image_out2 = draw_instance(image_path, keypoints_dict3, KEYPOINT_TYPES, hightlight_keypoint_types=episode_generator.support_kp_categories, save_root=None, is_show=False)
            # image_out2 = draw_instance(image_path, keypoints_dict3, KEYPOINT_TYPES, hightlight_keypoint_types=keypoints_dict3.keys(), save_root=None, is_show=False)


            image = cv2.imread(image_path)
            image_out = draw_skeletons(image, [keypoints_dict3], KEYPOINT_TYPES, marker='circle', circle_radius=10, thickness=-1, alpha=0.7)
        ...
        if draw_interpolated_kps == True:
            keypoints = each_sample['keypoints']  # kp_type: (x, y, is_visible)
            # all_labels = [[keypoints[kp_type][0], keypoints[kp_type][1], keypoints[kp_type][2]] for kp_type in KEYPOINT_TYPES]  # (x, y, is_visible)
