...
def feature_modulator(support_features, support_labels, support_kp_mask, query_features, context_mode='soft_fiber_bilinear', sigma=13,
    ...
    if context_mode == 'hard_fiber':
        ...
        # scale the label (x, y) which ranges -1~1 (continuous) to 0~image_width (continous), and then convert to discrete integer
        support_labels = (support_labels / 2 + 0.5).mul(W).long().clamp(0, W - 1)
        support_embeddings = []
        for i in range(B1):
...
def extract_representations(features, labels, kp_mask, context_mode='soft_fiber_bilinear', sigma=13, downsize_factor=16, image_length=368, together_trans_features=None):
    ...
    if context_mode == 'hard_fiber':
        # method 1, hard quantization
        # scale the label (x, y) which ranges -1~1 (continuous) to 0~image_width (continous), and then convert to discrete integer
        labels = (labels/2 + 0.5).mul(W).long().clamp(0, W - 1)
        embeddings = []
        for i in range(B1):
    ...
    # ====================
        ...
        kp_label_original = ((labels / 2 + 0.5) * original_width).long().clamp(0, original_width-1) # original image length 384
        for sample_i in range(B1):
            for kp_j in range(N_categories):
                if kp_mask[sample_i, kp_j] == 0:
                    # continue  # will cause the whole heatmap to be zero; then induce extracted kp fiber to be zero.
                    kp_label_original[sample_i, kp_j, :] = int((0 / 2 + 0.5) * original_width)  # set center (c, c) to (x, y)

                # print(sample_i, kp_j)
                # kp_label_original = (labels_normalized[sample_i, kp_j, :] * (original_width-1))
                # heatmaps[sample_i, kp_j, :, :], return_flag = putGaussianMaps(kp_label_original, sigma, H, W, stride, normalization=True)
                heatmaps[sample_i, kp_j, :, :], return_flag = putGaussianMaps(kp_label_original[sample_i, kp_j, :], sigma, H, W, stride, normalization=True)
                # if return_flag == False:
                #     print_weights(support_kp_mask)
                #     print_weights(support_labels)
                #     print(sample_i, kp_j)
...
def feature_modulator_individual(support_fibers, query_features, output_modulated_features=False, attention_type=None, output_attention_maps=False, compute_similarity=False):
    '''
    compute attention features for each individual feature
    :param support_fibers: B x C x N, each image has N fibers
    :param query_feature: B x C x H x W
    :param fused_attention:
    :param output_attention_maps:
    :param compute_similarity:
    :return: modulated features (B x N x C x H x W) and N attention maps (B x N x H x W)
    '''
    _, _, N = support_fibers.shape
    B, C, H, W = query_features.shape

    if output_modulated_features:
        attentive_features = query_features.reshape(B, C, H*W).unsqueeze(dim=1) * support_fibers.permute(0, 2, 1).reshape(B, N, C, 1)  # B x N x C x (H*W)
        attentive_features = attentive_features.reshape(B, N, C, H, W)  # modulated attentive features, B x N x C x H x W
    else:
        attentive_features = None

    if output_attention_maps == True and output_modulated_features == True:
        # l2 norm
        support_fibers_l2 = torch.norm(support_fibers, dim=1)  # B x N
        query_features_l2 = torch.norm(query_features, dim=1)  # B x H x W
        attention_maps_l2 = attentive_features.sum(dim=2) / (query_features_l2.view(B, 1, H, W) * support_fibers_l2.view(B, N, 1, 1) + 1e-9)

        # softmax
        attention_maps_l2 = nn.functional.softmax(attention_maps_l2.flatten(2), dim=2)    # B x N x (H*W)
        attention_maps_l2 = attention_maps_l2.reshape(B, N, H, W)  # B x N x H x W
    ...
    if compute_similarity == True:
        # normed_similarity_map are l2 normed similarity map
        # support_fibers_L2 = torch.norm(support_fibers, dim=1)  # B x N
        # query_features_L2 = torch.norm(query_features, dim=1)  # B x H x W
        # inner_product = (query_features.reshape(B, C, 1, H, W) * support_fibers.reshape(B, C, N, 1, 1)).sum(dim=1)
        # normed_similarity_map = inner_product / (query_features_L2.reshape(B, 1, H, W) * support_fibers_L2.reshape(B, N, 1, 1) + 1e-6)

        support_fibers_normalize = torch.nn.functional.normalize(support_fibers, p=2, dim=1)  # B x C x N
        query_features_normalize = torch.nn.functional.normalize(query_features, p=2, dim=1)  # B x C x H x W
        normed_similarity_map = (query_features_normalize.reshape(B, C, 1, H, W) * support_fibers_normalize.reshape(B, C, N, 1, 1)).sum(dim=1)
    ...
    return attentive_features, attention_maps_l2_cpu, normed_similarity_map
...
class DescriptorNet(nn.Module):
    def __init__(self, input_channel = 512, conv_blocks = 4, output_fiber=True, specify_fc_out_units = None, architecture_type = 'type1', net_config=(256, 1024), stride_pad=(2, 1)):
        ...
        if self.architecture_type == 'type1':
            ...
            conv_stride = stride_pad[0]
            conv_pad = stride_pad[1]
            self.convs = nn.Sequential()
            self.convs.add_module('block0_conv', nn.Conv2d(input_channel, mediate_channel, kernel_size=3, stride=conv_stride, padding=conv_pad)),
            self.convs.add_module('block0_relu', nn.ReLU(True))
            for i in range(conv_blocks - 2):
                self.convs.add_module('block%d_conv'%(i + 1), nn.Conv2d(mediate_channel, mediate_channel, kernel_size=3, stride=conv_stride, padding=conv_pad)),
                self.convs.add_module('block%d_relu'%(i + 1), nn.ReLU(True))
            self.convs.add_module('block%d_conv'%(conv_blocks-1), nn.Conv2d(mediate_channel, convs_out_channel, kernel_size=3, stride=conv_stride, padding=conv_pad)),
...
class GridBasedLocatorX2Covar(nn.Module):
    def __init__(self, grid_length=8, reg_uncertainty=True, cls_uncertainty=True, ureg_type=0, covar_Q_size=None, sample_times=15, negative_noise=False, sigma_computing_mode=0, alpha=1, beta=0, linear_in_channel=1024*2*2):
        ...
        linear_in_channel = linear_in_channel  # 1/32 (1024*2*2), 1/16 (1024*3*3), 1/46, 1/64, needs to be tuned manually
        # linear_in_channel = 1024        # 1/46, 1/64, needs to be tuned manually
        self.linear_grid_class = nn.Sequential(
            nn.Linear(linear_in_channel, grid_length * grid_length),
            # nn.LogSoftmax(dim=1)
...
class CovarNet(nn.Module):
    def __init__(self, grid_length=8, covar_Q_size=(2,2,2,3), ureg_type=3, linear_in_channel=1024 * 2 * 2):
        super(CovarNet, self).__init__()
        linear_in_channel = linear_in_channel  # 1/32, 1/46, 1/64, needs to be tuned manually
